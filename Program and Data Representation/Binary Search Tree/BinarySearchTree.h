...
class BinarySearchTree {
 ...
 private:
  ...
  bool find2(const string& x, BinaryNode* n) const;
  void inserting(const string& x, BinaryNode*& n);
  int counter(BinaryNode* n)const;
  string path2(const string& x, BinaryNode* n) const; 
  void clearTree(BinaryNode* n); // clears the tree
};
